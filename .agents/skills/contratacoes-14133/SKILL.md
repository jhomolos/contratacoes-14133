...
Para redigir o TR: identifique o tipo de objeto (compras vs. serviços/obras, TIC vs. não-TIC) para escolher o arquivo certo dentre os quatro acima, depois pergunte a modalidade (licitação ou contratação direta), **se a contratação será feita por Sistema de Registro de Preços (SRP)** — isso muda alternativas "OU" do próprio modelo (ex.: critério de aceitabilidade de preços unitários máximos por grupo de itens, e a adequação orçamentária, exigível no SRP apenas antes da assinatura do contrato, não na elaboração do TR) — e as características do caso (regime de execução, dedicação exclusiva de mão de obra, etc.) para saber quais alternativas "OU" do modelo usar. **Risque as alternativas não escolhidas, não as apague** — ver "Regra de ouro" acima.
